...
const steamAppId = computed(() => {
    ...
    return /^\d+$/.test(String(appid)) ? String(appid) : null;
});
...
const imageUrl = computed(() => {
    return imageUrls.value[
        Math.min(imageIndex.value, imageUrls.value.length - 1)
    ];
});
...
const gameTitle = computed(() => {
    return props.game.title ?? props.game.name ?? 'Unknown game';
});

const gameHref = computed(() => {
    const id = props.game.id ?? props.game.appid;
...
<template>
    <component
        :is="selectionMode ? 'div' : Link"
        :href="selectionMode ? undefined : gameHref"
        class="group flex h-full flex-col overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900 transition hover:border-zinc-700 hover:bg-zinc-800 sm:rounded-2xl"
        :class="selectionMode ? 'cursor-default' : ''"
    >
        ...
        <div
            class="flex flex-1 flex-col items-start justify-between gap-2 p-3 sm:flex-row sm:items-center sm:gap-3 sm:p-4"
        >
            <h3
                class="line-clamp-1 w-full text-xs font-bold text-white sm:text-sm"
            >
                {{ gameTitle }}
            </h3>
